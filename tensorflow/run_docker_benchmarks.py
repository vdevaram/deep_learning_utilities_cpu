###########################################################################################################
# This is a sample script file to run TF benchmarks with mkldnn library on Intel CPUs with Docker
...
#############################################################################################################
# sample kp.sh
#    docker ps -a | grep tf_mkl | for i in $(awk '{print $NF}');do docker stop $i -t 1; docker rm -f $i; done
#############################################################################################################
...
script_path = "/workspace/benchmarks/scripts/tf_cnn_benchmarks/tf_cnn_benchmarks.py"
...
data_params = ["/workspace/tf_records", "imagenet"]
protocol = "grpc"
# Path of the logfile to store output logs
log_path = "/workspace/tf_records/"
...
for model in models:
    idx = 0
    for node in ps_nodes:
        print 'ssh',node,"'./kd.sh'"
    for node in worker_nodes:
        print 'ssh',node,"'./kd.sh'"
    for node in ps_nodes:
        for numa in ps_numa_cmd[num_ps_per_node]:
            ps_name = "psdock" + str(idx)
            cmd = "docker run -t --privileged --network=host --name " + ps_name + " -v /home/vinod/work/tf/tf_records:/workspace/tf_records vdevaram/tf_mkl:1.5.0 /bin/bash -c "
            cmd = cmd + '" '
            for var in ps_envs:
                cmd = cmd + var + "; "
    ...
    for node in worker_nodes:
        for numa in wk_numa_cmd[num_workers_per_node]:
            wk_name = "wkdock" + str(idx)
            cmd = "docker run -t --privileged --network=host --name " + wk_name + " -v /home/vinod/work/tf/tf_records:/workspace/tf_records vdevaram/tf_mkl:1.5.0 /bin/bash -c "
            cmd = cmd + '" '
