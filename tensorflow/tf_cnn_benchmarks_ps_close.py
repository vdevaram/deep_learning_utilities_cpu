...
tf.flags.DEFINE_boolean('ps_auto_shut', False, 'close ps server automatically')
...
class BenchmarkCNN(object):
  ...
  def create_job_queue(self,i):
    """Queue used to signal death for i'th ps server. Intended to have
    all workers enqueue an item onto it to signal doneness."""

    with tf.device("/job:ps/task:%d" % (i)):
      return tf.FIFOQueue(len(self.worker_hosts), tf.int32, shared_name="job_queue"+ str(i))

  def create_job_queues(self):
    return [self.create_job_queue(i) for i in range(len(self.ps_hosts))]
  ...
  def run(self):
    if FLAGS.job_name == 'ps':
      ...
      if FLAGS.ps_auto_shut:
        sess = tf.Session(self.server.target)
        queue = self.create_job_queue(FLAGS.task_index)
  ...
  def _benchmark_cnn(self):
    ...
    enq_ops = []
    if FLAGS.ps_auto_shut:
      for q in self.create_job_queues():
        qop = q.enqueue(1)
        enq_ops.append(qop)
