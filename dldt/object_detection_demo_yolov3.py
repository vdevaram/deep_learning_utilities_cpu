...
def main():
    ...
    if in_image.shape[:-1] != (h, w):
        log.warning("Image {} is resized from {} to {}".format(args.input, in_image.shape[:-1], (h, w)))
        image = cv2.resize(in_image, (w, h))
    image = image.transpose((2, 0, 1))  # Change data layout from HWC to CHW

    #TBD VINOD in_frame = in_frame.reshape((n, c, h, w))
    ...
    for layer_name, out_blob in res.items():
        ...
        objects += parse_yolo_region(out_blob, image.shape[1:],
                                     in_image.shape[:-1], layer_params,
                                     args.prob_threshold)
    ...
    if len(objects):
        ...
        for obj in objects:
            ...
            log.info("{:^9} | {:10f} | {:4} | {:4} | {:4} | {:4} ".format(det_label, obj['confidence'], obj['xmin'], obj['ymin'], obj['xmax'], obj['ymax']))
